...
function applyDiscount(discount) {
  return function (price) {
    return price * (1 - discount); // use 'price', not 'product.price'
  };
}

const discount10 = applyDiscount(0.1); // 10% discount

const discountProducts = electronicproducts.map(product => ({
  ...product,
  price: discount10(product.price) // pass in the price, not the whole product
}));
